    ...
    return function (selector, config) {
      ...
      var _addIndicators = function () {
        ...
        for (var i = 0, length = _sliderItems.length; i < length; i++) {
          ...
          sliderIndicatorsItemName.classList.add('slider__indicators__name');
      ...
      var _setUpListeners = function () {
        ...
        if (_isTouchDevice()) {
          _slider.addEventListener('touchstart', function (e) {
            _startX = e.changedTouches[0].clientY;
            _startAutoplay();
          });
          _slider.addEventListener('touchend', function (e) {
            var
              _endX = e.changedTouches[0].clientY,
    ...
    var _move = function (direction) {
      ...
      if (direction === 'next') {
        ...
        if (_currentPosition > position.getItemPosition('max')) {
          ...
          _itemsArray[nextItem].position = position.getItemPosition('max') + 1;
          _itemsArray[nextItem].transform += _itemsArray.length * 100;
          console.log(_itemsArray[nextItem].transform);
          _itemsArray[nextItem].item.style.transform = 'translateY(' + _itemsArray[nextItem].transform + '%)';
        }
        _transformValue -= _transformStep;
        _indicatorIndex = _indicatorIndex + 1;
        ...
        _currentPosition--;
        console.log(position.getItemPosition('min'));
        if (_currentPosition < position.getItemPosition('min')) {
          nextItem = position.getItemIndex('max');
          _itemsArray[nextItem].position = position.getItemPosition('min') - 1;
          _itemsArray[nextItem].transform -= _itemsArray.length * 100;
      ...
      console.log(_sliderContainer.style.transform);
      _sliderContainer.style.transform = 'translateY(' + _transformValue + '%)';
      _indicatorItems[currentIndicator].classList.remove('active');
      _indicatorItems[_indicatorIndex].classList.add('active');
    };
...
var slideShow = (function () {
    return function (selector, config) {
      var
        _slider = document.querySelector(selector), // основный элемент блока
        _sliderContainer = _slider.querySelector('.slider__items-hobbies'), // контейнер для .slider-item
        _sliderItems = _slider.querySelectorAll('.slider__item-hobbies'), // коллекция .slider-item
        _sliderControls = _slider.querySelectorAll('.slider__controlv'), // элементы управления
        _currentPosition = 0, // позиция левого активного элемента
        _transformValue = 0, // значение транфсофрмации .slider_wrapper
        _transformStep = 100, // величина шага (для трансформации)
        _itemsArray = [], // массив элементов
      ...
      var _addIndicators = function () {
        var indicatorsContainer = document.createElement('ol');
        indicatorsContainer.classList.add('slider__indicators-hobbies');
        for (var i = 0, length = _sliderItems.length; i < length; i++) {
          var sliderIndicatorsItem = document.createElement('li');
          if (i === 0) {
            sliderIndicatorsItem.classList.add('active');
          ...
          var sliderIndicatorsItemName = document.createElement('span');
          sliderIndicatorsItemName.classList.add('slider__indicators__name-hobbies');
          sliderIndicatorsItemName.textContent = _itemsName[i];
          sliderIndicatorsItem.setAttribute("data-slide-to-hobbies", i);
          indicatorsContainer.appendChild(sliderIndicatorsItem);
          sliderIndicatorsItem.appendChild(sliderIndicatorsItemName);
        }
        _slider.appendChild(indicatorsContainer);
        _indicatorItems = _slider.querySelectorAll('.slider__indicators-hobbies > li')
      };
      ...
      var _setUpListeners = function () {
        ...
        if (_isTouchDevice()) {
          ...
          for (var i = 0, length = _sliderControls.length; i < length; i++) {
            _sliderControls[i].classList.add('slider__control_show-hobbies');
          }
        }
        _slider.addEventListener('click', function (e) {
          if (e.target.classList.contains('slider__control-hobbies')) {
            e.preventDefault();
            _move(e.target.classList.contains('slider__control_next-hobbies') ? 'next' : 'prev');
            _startAutoplay();
          } else if (e.target.getAttribute('data-slide-to-hobbies')) {
            e.preventDefault();
            _moveTo(parseInt(e.target.getAttribute('data-slide-to-hobbies')));
            _startAutoplay();
          }
        });
        document.addEventListener('visibilitychange', function () {
